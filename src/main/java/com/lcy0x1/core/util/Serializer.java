...
public class Serializer {
    ...
    public static class RLClassHandler<T> extends ClassHandler<T> {
        ...
        public RLClassHandler(Class<T> cls, Supplier<IForgeRegistry<T>> r) {
            super(cls, e -> e.isJsonNull() ? null : r.get().getValue(new ResourceLocation(e.getAsString())),
                    ...
                    (p, t) -> p.writeUtf(t == null ? "" : getName(cls, t).toString()));
        }

    }

    private static ResourceLocation getName(Class c, Object t) {
        if (c == Item.class) return ForgeRegistries.ITEMS.getKey((Item) t);
        if (c == Block.class) return ForgeRegistries.BLOCKS.getKey((Block) t);
        if (c == Potion.class) return ForgeRegistries.POTIONS.getKey((Potion) t);
        return null;
    }
