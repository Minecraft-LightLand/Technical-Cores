...
public class LangFileOrganizer extends ResourceOrganizer {
    ...
    public void organize(File f) throws Exception {
        for (File fi : list(f)) {
            ...
            JsonElement root_json = new JsonParser().parse(new FileReader(fi.getPath() + "/main.json", StandardCharsets.UTF_8));
            JsonObject dst_json = new JsonObject();
            for (File fj : list(fi)) {
                if (fj.getName().startsWith("."))
                    continue;
                JsonObject part = new JsonParser().parse(new FileReader(fj, StandardCharsets.UTF_8)).getAsJsonObject();
                if (part.has("simple")) {
